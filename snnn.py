...
class SpikingConv:
    ...
    def __init__(self, input_shape, out_channels, kernel_size, stride, padding=0, 
        ...
        # STDP
        self.pre_trace_prev=None
        self.recorded_spks = np.zeros((in_channels, in_height+2*self.padding[0], in_width+2*self.padding[1]))
        self.pre_trace=np.zeros(((in_channels, in_height+2*self.padding[0], in_width+2*self.padding[1])))
        ...
        # 在 __init__ 里加一行：抑制后留的安全余量（浮点用）
        self.inhibit_margin = 0.05  # 让被抑制电位落在 0.95*thr 以下
    ...
    '''def get_winners(self):
        ...
        while len(winners) < self.nb_winners:
            for c in range(C): 
                ...
                for h in range(H):
                    for w in range(W):
                        if pots_tmp[c,h,w] < self.firing_threshold :
                            continue
                        selected_c.append(c)
                        winners.append((c,h,w))
                        if len(winners) >= self.nb_winners:
                            return winners
        return winners'''
    def get_winners(self, spk_mask: np.ndarray):
        # spk_mask: 本步已放电(并且允许学习)的候选，形状和 self.pot 一样
        cand = spk_mask & self.stdp_neurons
        idx = np.array(np.where(cand)).T  # (N, 3)
        if len(idx) == 0:
            return []
    ...
    def lateral_inhibition(self, spks, r):#(不改变原来的spks，只调整pot)
        if not np.any(spks):
            return
        ...
        ys, xs = np.where(spks.any(axis=0))  # 有任意通道放电的 (h,w)

        for h, w in zip(ys, xs):
            # 对“所有在该点位放电的通道”，各自做同通道邻域软抑制
            cs = np.where(spks[:, h, w])[0]
            for c in cs:
                h0, h1 = max(0, h - r), min(H, h + r + 1)
                w0, w1 = max(0, w - r), min(W, w + r + 1)
                self.pot[c, h0:h1, w0:w1] =np.maximum(0,self.pot[c, h0:h1, w0:w1]-0.1*self.firing_threshold)
            # 可选：跨通道同点位轻衰减（避免完全并发但不粗暴）
            others = np.setdiff1d(np.arange(C), cs, assume_unique=True)
            if len(others) > 0:
                self.pot[others, h, w] =np.maximum(0,self.pot[others, h, w]-0.5*self.firing_threshold)
    ...
    def stdp(self, winner):
        if not self.stdp_neurons[winner]: return
        ...
        conv=self.get_conv_of(self.pre_trace,winner).flatten()
        # Compute dW
        w = self.weights[winner_c].flatten() * (1 - self.weights[winner_c]).flatten()
        eps = 1e-6 
        w_plus = conv > eps # Pre-then-post
        w_minus = conv <= eps # Post-then-pre (we assume that if no spike before, then after)
        ...
        # Lateral inhibition in the same channel (gobal intra competition)(取消整个通道禁用，只禁邻域)
        #self.stdp_neurons[winner] = False
        # --- 仅禁本通道小邻域的 STDP 资格 ---
        r = int(self.inhibition_radius)
        H, W2 = self.pot.shape[1], self.pot.shape[2]
        h0, h1 = max(0, winner_h - r), min(H, winner_h + r + 1)
        w0, w1 = max(0, winner_w - r), min(W2, winner_w + r + 1)
        self.stdp_neurons[winner_c, h0:h1, w0:w1] = False
    ...
    def __call__(self, spk_in, train=False):
        # padding 
        spk_in = np.pad(spk_in, ((0,0), (self.padding[0],self.padding[0]), (self.padding[1],self.padding[1])), mode='constant')
        ...
        delay=0.875
        self.pre_trace*=delay
        self.pre_trace+=spk_in
        if np.any(output_spikes):
            ...
            self.lateral_inhibition(spk_out,self.inhibition_radius)
            # STDP plasticity
            if train and self.plasticity:
                # Find winners (based on potential)
                winners = self.get_winners(spk_out.astype(bool))
                # Apply STDP for each neuron winner
                for winner in winners:
                    self.stdp(winner)
                winners_mask = np.zeros_like(spk_out, bool)
                for c,h,w in winners:
                    winners_mask[c,h,w]=True
